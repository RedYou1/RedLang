...
namespace DTO {
	...
	public:
		System() :Class(L"System", Paths::System, GLOBAL::getClasses()->getClass(Paths::Object)) {
		}
		...
		public:
			CommandReturn* exec(MemoryObject& mem) {
				IObject* s{ mem.get(L"s") };
				Instanciable** cc{ new Instanciable * [1] {s->getClass()} };
				Function* func{ s->getClass()->getFuncs()->get(L"toString",cc,1) };
				delete[] cc;
				IObject** cmds{ new IObject * [1] {s} };
				CommandReturn* r{ func->exec(mem, cmds,1) };
				delete[] cmds;
				std::wcout << ((StringO*)r->getObject())->m_value;
				delete r;
		...
		public:
			CommandReturn* exec(MemoryObject& mem) {
				...
				CommandReturn* r{ func->exec(mem, cmds,1) };
				delete[] cmds;
				std::wcout << ((StringO*)r->getObject())->m_value << std::endl;
				delete r;
				return new CommandReturn(new NullObject(), true, false);
			}
