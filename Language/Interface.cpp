...
Interface::Interface(std::string name, std::string path)
	:SourceFile(name, path), m_genTypes(new MemorySourceFile{ false }), m_interfaces(new Interface* [0]), m_interfaceLen(0), m_signs()
{
}

Interface::Interface(std::string name, std::string path, MemorySourceFile* genTypes)
	: SourceFile(name, path), m_genTypes(genTypes), m_interfaces(new Interface* [0]), m_interfaceLen(0), m_signs()
{
}

Interface::Interface(std::string name, std::string path, MemorySourceFile* genTypes, Interface** implements, size_t implementsLen)
...
void Interface::add(std::string name, Signature* o)
{
...
std::list<Signature*> Interface::get(std::string name)
{
	typename std::map<std::string, std::list<Signature*>>::iterator it(m_signs.find(name));
	if (it == m_signs.end()) {
...
Signature* Interface::get(std::string name, Interface** argsType, size_t argsLen)
{
...
bool Interface::containsAll(Class* _class)
{
...
bool Interface::instanceOf(Interface* other)
{
	if (other == nullptr)
		return true;
	if (this == nullptr)
...
Interface* Interface::parse(std::string path, std::string str, MemorySourceFile& genTypes)
{
	myString s{ &str };
