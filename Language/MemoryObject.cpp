...
void MemoryObject::add(std::string name, IObject* o, Interface* type) {
	if (m_vars.find(name) != m_vars.end())
		throw "variable already exists";
	m_vars.insert(std::pair<std::string, Memory*>(name, new Memory{ o ,type }));
}

void MemoryObject::set(std::string name, IObject* o) {
...
IObject* MemoryObject::get(std::string name) {
	typename std::map<std::string, Memory*>::iterator it(m_vars.find(name));
	if (it == m_vars.end()) {
		if (m_parent != nullptr)
...
MemoryObject::Memory::Memory(IObject* object, Interface* type)
...
IObject* MemoryObject::Memory::getObject()
{
	return m_object;
}
...
void MemoryObject::Memory::setObject(IObject* object)
{
	if (m_object == object)
		return;
	GarbageCollector::Remove(m_object);
